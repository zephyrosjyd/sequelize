...
class Association {
  constructor(source, target, options = {}) {
    ...
    if (this.as) {
      ...
      this.as = this.target.options.name;
     ...
     * @type {string}
     */
    this.associationType = this.constructor.name;

    if (source.hasAlias(this.as)) {
      throw new AssociationError(`You have used the alias ${this.as} in two separate associations. ` +
  ...
  mixin(prototype) {
    const assoc = this;
    if (!('associations' in prototype)) {
      prototype._associator = class MyProxy extends SomethingProxy { };
      Object.defineProperty(prototype, 'associations', {
        enumerable: true,
        get() {
          return new this._associator(assoc, this);
        }
      });
    }
    Object.defineProperty(prototype._associator.prototype, this.as, {
      enumerable: true,
      get() {
        return assoc.getForInstance(this.model);
      }
    });
  }
...
class AssociationInstance {
  constructor(association, instance) {
    this.association = association;
    this.instance = instance;
  }
}

class OneToOneAssociationInstance extends AssociationInstance {
  get(options) {
    return this.association.get(this.instance, options);
  }

  set(associatedInstance, options) {
    return this.association.set(this.instance, associatedInstance, options);
  }

  create(values, options) {
    return this.association.set(this.instance, values, options);
  }
}

class OneToManyAssociationInstance extends AssociationInstance {
  get(options) {
    return this.association.get(this.instance, options);
  }

  count(options) {
    return this.association.count(this.instance, options);
  }

  set(newAssociatedObjects, options) {
    return this.association.set(this.instance, newAssociatedObjects, options);
  }

  add(newInstances, options) {
    return this.association.add(this.instance, newInstances, options);
  }

  remove(oldAssociatedObjects, options) {
    return this.association.remove(this.instance, oldAssociatedObjects, options);
  }

  create(values, options) {
    return this.association.create(this.instance, values, options);
  }
}


exports.Association = Association;
exports.OneToOneAssociationInstance = OneToOneAssociationInstance;
exports.OneToManyAssociationInstance = OneToManyAssociationInstance;
