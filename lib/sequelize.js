...
class Sequelize {
   ...
   *
   * ```js
   * const [results, metadata] = await sequelize.query('SELECT...'); // Raw query - use spread
   *
   * const results = await sequelize.query('SELECT...', { type: sequelize.QueryTypes.SELECT }); // SELECT query - no spread
   * ```
   *
   * @param {string}          sql
   ...
   *
   * try {
   *   await sequelize.transaction(transaction => { // Note that we use a callback rather than a await
   *     const user = await User.findOne(..., {transaction});
   *     await user.update(..., {transaction});
   *   });
   *   // Committed
