...
/**
 * Calculates string prefix eg. `a.b.c` => `a.b`.
 *
 * @param {string} str
 */
const getPrefix = str => str.substr(0, str.lastIndexOf('.'));
...
const makeArrayPrefixMemoizer = () => {
  const keyPrefixMemo = {};
  return key => {
    if (!Object.prototype.hasOwnProperty.call(keyPrefixMemo, key)) {
      const prefix = getPrefix(key);
      keyPrefixMemo[key] = prefix ? prefix.split('.') : [];
    }
...
class AbstractQuery {
  ...
  static _groupJoinData(rows, includeOptions, options) {
    ...
    let previousPiece;
    const buildIncludeMap = (key, piece) => {
    ...
    const arrayPrefix = makeArrayPrefixMemoizer();

    // Compute hash key for this set instance
    // TODO: Optimize
    const computeHashKeyForInstance = () => {
      ...
      if (length) {
        for (let i = 0; i < length; i++) {
          ...
          const primaryKeyAttributes = includeMap[prefix].model.primaryKeyAttributes;
          const $length = primaryKeyAttributes.length;
          itemHash = prefix;
          if ($length > 0) {
            for (const primKeyAttr of primaryKeyAttributes) {
    ...
    const keys = rows.length > 0 ? Object.keys(rows[0]) : undefined;
    for (let rowsI = 0; rowsI < rowsLength; rowsI++) {
      ...
      if (checkExisting) {
        ...
        topHash = '';
        if (includeOptions.model.primaryKeyAttributes.length > 0) {
          for (const primKeyAttribute of includeOptions.model.primaryKeyAttributes) {
            topHash += stringify(row[primKeyAttribute]);
          }
        }
      }
      ...
      for (const key of keys) {
        ...
        const $keyPrefix = arrayPrefix(key);
        ...
        if (rowsI === 0 && !Object.prototype.hasOwnProperty.call(includeMap, key)) {
          if (!$keyPrefix.length) {
            ...
            for (const piece of $keyPrefix) {
              buildIncludeMap(key, piece);
            }
          }
        }
        // End of key set
